...
**Drift** is a same-origin snapshot at [`/drift/`](https://apps.kulibert.net/drift/) (`drift/` in this repo). Do not rewrite it to `*.vercel.app` — Solvay blocks that host. HTML/JS/CSS stay under `/drift/assets/…`. Prove: `curl -sS https://apps.kulibert.net/drift/ | grep vercel.app` must be empty. Rebuild: `bash scripts/publish-drift.sh`.
