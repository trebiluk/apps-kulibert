...
3. Student storage may keep a beat title only. No names.
...
- Lights use the shared stage (`/shared/visualizer/stage.js`). Same four Looks.
- Export downloads the beat as `.json` or `.txt`. Import reads either. Save still stays on this Chromebook.
- Visualizer is its own door. Chip **Viz 0.1.0**.
- Hub chip **BZ 1.4.0**.

Hard refresh if a cart still says 1.3.0.
...
- Play rings the loop and brightens the moving column. Same beat. No new modes.
- What's new is one line in How.
- Hub chip **BZ 1.2.2**.

Hard refresh if a cart still says 1.2.1.
...
- 16-step grid, kits, mood, key, visualizer, local save, starter beats.
